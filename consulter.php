...
">
	<div class="container-fluid">
		<div class="row">
			...
			<?php 
			if (isset($_GET['id'])){
			$sql="SELECT codeuses.id, nom, prenoms, date_naissance, telephone, email, description, photo, specialisation, facebook, twitter, github FROM cv INNER JOIN codeuses ON codeuses.id=cv.id_codeuse
		...
		<?php 
if (isset($_GET['id'])){
			$sql="SELECT codeuses.id, date_debut, date_fin, poste, experiences.description, entreprise FROM experiences INNER JOIN codeuses ON codeuses.id= experiences.id_codeuse
			WHERE codeuses.id=".$_GET['id'];
			$res=mysqli_query($link, $sql);
